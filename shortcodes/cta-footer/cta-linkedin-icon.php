<?php
/**
 * Create a simple shortcode and name it as [cta_linkedin_icon]
 * - attributes are:
 * - - href (required) - this is the href of a tag
 */
add_shortcode( 'cta_linkedin_icon', function ( $atts ) {
	$atts = shortcode_atts(
		[
			'href' => 'https://www.linkedin.com/company/supply-chain-now/',
		],
		$atts,
		'cta_linkedin_icon'
	);

	$href = trim( (string) $atts['href'] );
	if ( $href === '' ) {
		return '';
	}

	$href_esc     = esc_url( $href );
	$link_classes = 'text-white hover:text-primary w-inline-block';

	return
		'<a href="' . $href_esc . '" class="' . esc_attr( $link_classes ) . '">' .
		'<div class="flex items-center justify-center w-embed">' .
		'<svg xmlns="http://www.w3.org/2000/svg" width="37" height="36" viewBox="0 0 37 36" fill="none">' .
		'<path fill-rule="evenodd" clip-rule="evenodd" d="M0.411255 18C0.411255 8.05888 8.47013 0 18.4113 0C28.3524 0 36.4113 8.05888 36.4113 18C36.4113 27.9411 28.3524 36 18.4113 36C8.47013 36 0.411255 27.9411 0.411255 18ZM9.63625 14.9625V27H13.4613V14.9625H9.63625ZM9.41125 11.1375C9.41125 12.375 10.3113 13.275 11.5488 13.275C12.7863 13.275 13.6863 12.375 13.6863 11.1375C13.6863 9.9 12.7863 9 11.5488 9C10.4238 9 9.41125 9.9 9.41125 11.1375ZM23.5863 27H27.1863V19.575C27.1863 15.8625 24.9362 14.625 22.7987 14.625C20.8863 14.625 19.5363 15.8625 19.1988 16.65V14.9625H15.5988V27H19.4237V20.5875C19.4237 18.9 20.5487 18 21.6737 18C22.7987 18 23.5863 18.5625 23.5863 20.475V27Z" fill="currentColor"></path>' .
		'</svg>' .
		'</div>' .
		'</a>';
} );