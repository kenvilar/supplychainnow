...
if ( ! function_exists( 'scn_searchbar_shortcode' ) ) {
  function scn_searchbar_shortcode( $atts = [] ) {
    ...
    $atts     = shortcode_atts( $defaults, $atts, 'searchbar' );

    $pageID             = get_the_ID();
    $site_padding       = $atts["site_padding"] ?? "";
    ...
    $passed_taxonomy    = isset( $atts['taxonomy'] ) ? sanitize_key( $atts['taxonomy'] ) : null;
    $hide_dropdown      = isset( $atts['hide_dropdown'] ) ? $atts['hide_dropdown'] : false;
    $placeholder        = isset( $atts['placeholder'] ) ? $atts['placeholder'] : 'Search by episode, topic, name, etc...';
                ...
                <div class="relative overflow-hidden rounded-100 border border-secondary/50 bg-[#EBF6FF]">
                  <label>
                    <input
                      ...
                      name="search"
                      value="<?php
                      echo isset( $_GET['search'] ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['search'] ) ) ) : ''; ?>"
                      placeholder="<?php
                      echo $placeholder; ?>"
