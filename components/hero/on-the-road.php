...
$pageID = get_the_ID();
$page_title = get_field("Page_Title", $pageID) ?: "On The Road";
$description = esc_html(get_field("Description", $pageID) ?: "");
$hero_image = esc_url(get_field("Hero_Image", $pageID) ?: get_stylesheet_directory_uri() . "/assets/img/hero-img/hero--on-the-road.avif");
$icon = esc_url(get_field("Icon", $pageID) ?: get_stylesheet_directory_uri() . "/assets/img/icons/on-the-road.svg");
...
<section class="section bg-cargogrey text-white rounded-b-100 sm:rounded-b-none">
	...
	<div class="absolute absolute--full w-full h-full">
		<img
			src="<?= $hero_image ?>"
			loading="lazy" alt="hero-on-the-road" class="image opacity-10">
	</div>
</section>